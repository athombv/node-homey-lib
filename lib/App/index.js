...
class App {
  ...
  async validate({
    ...
    if( Array.isArray(appJson.drivers) ) {
      ...
      for( let i = 0; i < appJson.drivers.length; i++ ) {
        ...
        // validate 'app.Json.drivers[].settings[] default values'
        if (driver && Array.isArray(driver.settings)) {
          for (let j = 0; j < driver.settings.length; j++) {
            let setting = driver.settings[j];
            ...
            if (setting.type &&
              ...
              for (let k = 0; k < setting.children.length; k++) {
                ...
                if ( (typeof childSetting.min === 'number' && typeof childSetting.max === 'number') || (childSetting.attr && typeof childSetting.attr.min === 'number' && typeof childSetting.attr.max === 'number') ) {
                  this._checkDefaultForMinMaxSetting(driver, childSetting);
                } else if (childSetting.values) {
                  this._checkDefaultForArraySetting(driver, childSetting);
                }
              }
            } else {
              if ( (typeof setting.min === 'number' && typeof setting.max === 'number') || (setting.attr && typeof setting.attr.min === 'number' && typeof setting.attr.max === 'number') ) {
                this._checkDefaultForMinMaxSetting(driver, setting);
              } else if (setting.values) {
                this._checkDefaultForArraySetting(driver, setting);
              }
            }
          }
        }
  ...
  _checkDefaultForMinMaxSetting(driver, setting) {
    if (!driver || !setting) return;
    let min, max, defaultValue;
    typeof setting.attr.min === 'number' ? min = setting.attr.min : min = setting.min;
    typeof setting.attr.max === 'number' ? max = setting.attr.max : max = setting.max;

    if (typeof setting.value === 'number') {
      defaultValue = setting.value;

      if (defaultValue < min) throw new Error(`Setting: ${setting.id} in driver: ${driver.id} default value too low.`);
      if (defaultValue > max) throw new Error(`Setting: ${setting.id} in driver: ${driver.id} default value too high.`);
    } else throw new Error(`Setting: ${setting.id} in driver: ${driver.id} missing default value.`);
  }

  _checkDefaultForArraySetting(driver, setting) {
    if (!driver || !setting) return;
    let allowedValues = [], defaultValue;

    for (let i = 0; i < setting.values.length; i++) {
      allowedValues.push(setting.values[i].id);
    }

    if (typeof setting.value === 'number' || typeof setting.value === 'string') {
      defaultValue = setting.value;
      if (!allowedValues.includes(defaultValue)) throw new Error(`Setting: ${setting.id} in driver: ${driver.id} default value not allowed.`);
    } else throw new Error(`Setting: ${setting.id} in driver: ${driver.id} missing default value.`);
  }
