...
class App {
  ...
  async validate({
    ...
    if (appJson.esm === true && semver.lt(semver.coerce(appJson.compatibility), '12.1.0')) {
      throw new Error('ESM apps require a compatibility of at least >=12.1.0');
    }

    const checkMjs = async (path) => {
      await fs.promises.access(path).then(() => {
        if (semver.lt(semver.coerce(appJson.compatibility), '12.1.0')) {
          throw new Error(`ESM apps require a compatibility of at least >=12.1.0. (${path})`);
        }
      }).catch(err => {
        if (err.code !== 'ENOENT') {
          throw err;
    ...
    const appFilePath = join(this._path, 'app.mjs');
    const apiFilePath = join(this._path, 'api.mjs');

    await checkMjs(appFilePath);
    await checkMjs(apiFilePath);

    if (Array.isArray(appJson.drivers)) {
      for (const driver of appJson.drivers) {
        const driverFilePath = join(this._path, 'drivers', driver.id, 'driver.mjs');
        const deviceFilePath = join(this._path, 'drivers', driver.id, 'device.mjs');

        await checkMjs(driverFilePath);
        await checkMjs(deviceFilePath);
      }
    }

    if (Array.isArray(appJson.widgets)) {
      for (const widget of appJson.widgets) {
        const apiFilePath = join(this._path, 'widgets', widget.id, 'api.mjs');
