...
class App {
  ...
  async validate({
    ...
    if ((appJson.platforms || []).includes('local') === false
      && (appJson.platformLocalOptionalFeatures || []).length > 0) {
      console.warn('Warning: using `platformLocalOptionalFeatures` requires `platforms: [local]`.');
    }
    ...
    if (Array.isArray(appJson.drivers)) {
      ...
      for (let i = 0; i < appJson.drivers.length; i++) {
        ...
        if (driver.capabilitiesOptions) {
          for (const [capabilityId, capabilityOptions] of Object.entries(driver.capabilitiesOptions)) {
            ...
            if (Capability.isInstanceOfId(capabilityId, 'target_power_mode') && Array.isArray(capabilityOptions.values)) {
              ...
              if (!hasHomey) {
                throw new Error(`drivers.${driver.id}.capabilitiesOptions.${capabilityId}.values must include "homey" value`);
              }

              const nonHomeyValues = capabilityOptions.values.filter(v => v && typeof v === 'object' && typeof v.id === 'string' && v.id !== 'homey');
              if (nonHomeyValues.length === 0) {
                throw new Error(`drivers.${driver.id}.capabilitiesOptions.${capabilityId}.values must include at least one non-homey value`);
              }

              const RESERVED_PREFIXES = ['homey_'];
              for (const value of capabilityOptions.values) {
                if (value && typeof value === 'object' && typeof value.id === 'string'
                  && RESERVED_PREFIXES.some(prefix => value.id.startsWith(prefix))) {
                  throw new Error(`drivers.${driver.id}.capabilitiesOptions.${capabilityId}.values custom values cannot use reserved prefix "homey_": ${value.id}`);
                }
              }
            }
        ...
        if (typeof driver.energy === 'object') {
          if (driver.energy.gridConnection !== undefined && semver.lt(appCompatibilityMinVersion, '13.6.0')) {
            throw new Error(`drivers.${driver.id}.energy.gridConnection requires a compatibility of at least >=13.6.0`);
          }

          if (driver.energy.phases !== undefined && semver.lt(appCompatibilityMinVersion, '13.6.0')) {
            throw new Error(`drivers.${driver.id}.energy.phases requires a compatibility of at least >=13.6.0`);
          }

          if (driver.energy.gridConnection === true && (typeof driver.energy.phases !== 'object' || driver.energy.phases.l1 === undefined)) {
            throw new Error(`drivers.${driver.id} has 'energy.gridConnection' set to true, but is missing 'energy.phases.l1'.`);
          }

          if (typeof driver.energy.phases === 'object') {
            const phaseIds = ['l1', 'l2', 'l3'];
            const phaseCapabilityBaseIds = {
            ...
            phaseIds.forEach((phaseId, index) => {
              const phase = driver.energy.phases[phaseId];
              if (phase === undefined) return;

              const previousPhaseId = phaseIds[index - 1];
              if (previousPhaseId !== undefined && driver.energy.phases[previousPhaseId] === undefined) {
                throw new Error(`drivers.${driver.id} has 'energy.phases.${phaseId}' but is missing 'energy.phases.${previousPhaseId}'.`);
              }

              for (const [property, baseId] of Object.entries(phaseCapabilityBaseIds)) {
                if (Capability.isInstanceOfId(phase[property], baseId) === false) {
                  throw new Error(`drivers.${driver.id} has 'energy.phases.${phaseId}.${property}': '${phase[property]}' but only instances of '${baseId}' are allowed.`);
                }
              }
            });
          }
