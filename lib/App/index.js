...
class App {
	...
	async validate({
		...
		if( appJson.brandColor ) {
  		const color = tinycolor(appJson.brandColor);
  		const brightness = color.getBrightness() / 256;
  		if( brightness > 0.72 ) // empirically determined by many colorpicker samples
	...
	async _validateModules() {
  	...
  	const nodeModulesPath = path.join(this._path, 'node_modules');
    ...
    try {
      ...
      return;
		...
		for (let file of await readDirAsync(nodeModulesPath)) {
			let stat;
			try {
				stat = await lstatAsync(path.join(nodeModulesPath, file));
			} catch(e) {
