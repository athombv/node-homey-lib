    ...
    if (Array.isArray(appJson.drivers)) {
      ...
      for (let i = 0; i < appJson.drivers.length; i++) {
        ...
        if (typeof driver.energy === 'object') {
          ...
          if (driver.energy.currentLimitCapability !== undefined) {
            if (Capability.isInstanceOfId(driver.energy.currentLimitCapability, 'current_limit') === false) {
              // eslint-disable-next-line max-len
              throw new Error(`drivers.${driver.id} has 'energy.currentLimitCapability': '${driver.energy.currentLimitCapability}' but only instances of 'current_limit' are allowed.`);
            }
          }
