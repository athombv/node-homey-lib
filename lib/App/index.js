    ...
    if (Array.isArray(appJson.drivers)) {
      ...
      for (let i = 0; i < appJson.drivers.length; i++) {
        ...
        if (driver.connectivity && driver.connectivity.includes('matter')) {
          if (!driver.matter) {
            throw new Error(`drivers.${driver.id} invalid 'connectivity': 'matter' requires a 'matter' object.`);
          }

          if (driver.pair) {
            throw new Error(`drivers.${driver.id} invalid 'pair': matter drivers do not support custom pairing views.`);
          }
          ...
          if (driverJsExists) {
            throw new Error(`drivers.${driver.id}: using a driver.js file is not supported for Matter drivers.`);
          }
        }

        if (driver.matter && !(driver.connectivity && driver.connectivity.includes('matter'))) {
          throw new Error(`drivers.${driver.id} invalid 'matter': 'matter' requires 'connectivity' to include 'matter'.`);
        }
      }
    }
