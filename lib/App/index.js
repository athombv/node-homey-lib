    ...
    if (Array.isArray(appJson.drivers)) {
      ...
      for (let i = 0; i < appJson.drivers.length; i++) {
        ...
        if (driver.firmwareUpdates && Array.isArray(driver.firmwareUpdates.updates)) {
          ...
          const isZigbeeDriver = typeof driver.zigbee === 'object';
          ...
          for (const [updateIndex, update] of Object.entries(driver.firmwareUpdates.updates)) {
            if (typeof update.changelog !== 'string' && typeof update.changelog.en !== 'string') {
              throw new Error(`drivers.${driver.id}.firmwareUpdates.update[${updateIndex}] is missing a changelog string`);
            }

            if (!Array.isArray(update.files) || update.files.length === 0) {
              throw new Error(`drivers.${driver.id}.firmwareUpdates.update[${updateIndex}].files must include at least one file`);
            }
            ...
            if (isZigbeeDriver) {
              ...
              if (updateManufacturerNames.length === 0 || updateProductIds.length === 0) {
                throw new Error(`drivers.${driver.id}.firmwareUpdates.update[${updateIndex}] must include at least one manufacturerName and productId`);
              }
              ...
              if (hasMismatchedManufacturerName) {
                throw new Error(`drivers.${driver.id}.firmwareUpdates.update[${updateIndex}] has a manufacturerName that does not match the driver zigbee.manufacturerName`);
              }

              if (hasMismatchedProductId) {
                throw new Error(`drivers.${driver.id}.firmwareUpdates.update[${updateIndex}] has a productId that does not match the driver zigbee.productId`);
              }
            }
            ...
            for (const [fileIndex, file] of Object.entries(update.files)) {
              ...
              if (!isIntegrityValid) {
                throw new Error(`drivers.${driver.id}.firmwareUpdates.update[${updateIndex}].files[${fileIndex}] integrity mismatch`);
              }
              ...
              if (isZigbeeDriver) {
                try {
                  ...
                  throw new Error(`drivers.${driver.id}.firmwareUpdates.update[${updateIndex}].files[${fileIndex}] invalid Zigbee OTA header: ${err.message}`);
                }
              }

              const fileId = `${file.manufacturerCode}-${file.imageType}-${file.fileVersion}`;
              if (seenFiles.has(fileId)) {
                throw new Error(`drivers.${driver.id}.firmwareUpdates.update[${updateIndex}] has multiple files with the same manufacturerCode, imageType and fileVersion`);
              }
