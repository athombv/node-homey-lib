      ...
      for (let i = 0; i < appJson.drivers.length; i++) {
        ...
        if (driver.capabilitiesOptions) {
          for (const [capabilityId, capabilityOptions] of Object.entries(driver.capabilitiesOptions)) {
            // validate target_power_mode values must include 'device' and 'homey'
            if (Capability.isInstanceOfId(capabilityId, 'target_power_mode')) {
              if (Array.isArray(capabilityOptions.values)) {
                const valueIds = capabilityOptions.values.map(v => v.id);
                if (!valueIds.includes('device')) {
                  throw new Error(`drivers.${driver.id}.capabilitiesOptions.${capabilityId}.values must include value with id 'device'`);
                }
                if (!valueIds.includes('homey')) {
                  throw new Error(`drivers.${driver.id}.capabilitiesOptions.${capabilityId}.values must include value with id 'homey'`);
                }

                // validate custom value IDs don't use reserved 'homey' prefix
                const reservedIds = ['device', 'homey'];
                const invalidIds = valueIds.filter(id => !reservedIds.includes(id) && id.startsWith('homey'));
                if (invalidIds.length > 0) {
                  throw new Error(`drivers.${driver.id}.capabilitiesOptions.${capabilityId}.values contains custom value id(s) starting with reserved prefix 'homey': ${invalidIds.join(', ')}`);
                }
              }
            }
            ...
            if (Capability.isInstanceOfId(capabilityId, 'target_power')) {
              if (capabilityOptions.exclude && typeof capabilityOptions.exclude === 'object') {
                const excludeMin = typeof capabilityOptions.exclude.min === 'number' ? capabilityOptions.exclude.min : 0;
                const excludeMax = typeof capabilityOptions.exclude.max === 'number' ? capabilityOptions.exclude.max : 0;
                if (excludeMin > 0 || excludeMax < 0) {
                  throw new Error(`drivers.${driver.id}.capabilitiesOptions.${capabilityId}.exclude must include 0 (exclude.min <= 0 <= exclude.max)`);
                }
              }
            }
