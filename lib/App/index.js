...
class App {
  ...
  async validate({
    ...
    try {
      semver.Range(appJson.compatibility);
    } catch (err) {
      ...
      for (let i = 0; i < appJson.drivers.length; i++) {
        ...
        if (levelVerified) {
          ...
          if (driver.platforms.includes('cloud')) {
            ...
            if (driver.connectivity.includes('lan') || driver.connectivity.includes('rf868')) {
              throw new Error(`drivers.${driver.id} invalid 'connectivity': Homey Bridge does not support 'lan' or 'rf868'.`);
            }
          }
        }
  ...
  _validateFlowCard(card, errorPath, { levelPublish, levelVerified }) {
