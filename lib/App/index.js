...
class App {
  ...
  async validate({
    ...
    if (appJson.flow) {
      const doesNotSatisfyMultiselectVersion = semver.lt(
        semver.coerce(appJson.compatibility),
        "12.5.0"
      );
      ...
      for (const type of Object.keys(appJson.flow)) {
        ...
        if (Array.isArray(cards)) {
          for (let i = 0; i < cards.length; i++) {
            ...
            this._validateFlowCard(card, `flow.${type}['${card.id}']`, appJson, { levelPublish, levelVerified, doesNotSatisfyMultiselectVersion });
          }
        }
      }
    }
  ...
  _validateFlowCard(card, errorPath, appJson, { levelPublish, levelVerified, doesNotSatisfyMultiselectVersion }) {
    ...
    for (const argument of card.args) {
      if (argument.type === 'multiselect' && doesNotSatisfyMultiselectVersion) {
        throw new Error(
          `${errorPath}.args['${argument.name}'].type: 'multiselect' is not supported for compatibility < 12.5.0`
        );
      }
    }
