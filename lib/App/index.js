      ...
      for (let i = 0; i < appJson.drivers.length; i++) {
        ...
        if (driver.capabilitiesOptions) {
          for (const [capabilityId, capabilityOptions] of Object.entries(driver.capabilitiesOptions)) {
            // validate target_power_mode values must include 'automatic' and 'override'
            if (Capability.isInstanceOfId(capabilityId, 'target_power_mode')) {
              if (Array.isArray(capabilityOptions.values)) {
                ...
                if (!valueIds.includes('automatic')) {
                  throw new Error(`drivers.${driver.id}.capabilitiesOptions.${capabilityId}.values must include value with id 'automatic'`);
                }
                if (!valueIds.includes('override')) {
                  throw new Error(`drivers.${driver.id}.capabilitiesOptions.${capabilityId}.values must include value with id 'override'`);
                }
              }
            }
            ...
            // validate home battery target_power min/max range must include 0
            if (
              driver.energy && driver.energy.homeBattery === true &&
              Capability.isInstanceOfId(capabilityId, 'target_power')
            ) {
              ...
              if (min > 0 || max < 0) {
                throw new Error(`drivers.${driver.id}.capabilitiesOptions.${capabilityId}.min/max must include 0 (min <= 0 <= max) for home batteries to allow idle state`);
              }
            }
          }
        }
