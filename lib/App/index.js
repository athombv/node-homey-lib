        ...
        if (driver.capabilitiesOptions) {
          for (const [capabilityId, capabilityOptions] of Object.entries(driver.capabilitiesOptions)) {
            ...
            if (Capability.isInstanceOfId(capabilityId, 'target_power_mode')) {
              ...
              const rawValues = Array.isArray(capabilityOptions.values) ? capabilityOptions.values : [];
              const seenIds = new Set(canonicalValues.map(cv => cv.id));
              const extraValues = [];
              ...
              const reservedPrefixes = ['homey_', 'device_'];
              const reserved = extraValues.filter(v => reservedPrefixes.some(p => v.id.startsWith(p)));
              if (reserved.length) {
                throw new Error(`drivers.${driver.id}.capabilitiesOptions.${capabilityId} custom values cannot use reserved prefixes "homey_" or "device_": ${reserved.map(v => v.id).join(', ')}`);
              }
