...
class App {
	...
	async validate({
		...
		await this._ensureFileExistsCaseSensitive( path.join('README.md') );
    ...
    _checkZwaveForSetting(driver, setting) {
		if (!setting.zwave) return;
        else if (!setting.zwave.index || !setting.zwave.size) throw new Error(`Missing property in "zwave" at ${driver.id}, ${setting.id}`);
        else if ( (setting.attr && setting.attr.max) || setting.max ) {
			let signed;
			let max;
			let size = setting.zwave.size;

			setting.attr.max ? max = setting.attr.max : max = setting.max;
			setting.zwave.signed ? signed = setting.zwave.signed : signed = true;

			if (signed && size === 1 && max > 127) {
				throw new Error(`Value cannot be signed: ${driver.id}, ${setting.id}`);
			} else if (!signed && size === 1 && max > 255) {
				throw new Error(`Max value out of bounds: ${driver.id}, ${setting.id}`);
			}

			else if (signed && size === 2 && max > 32767) {
				throw new Error(`Value cannot be signed: ${driver.id}, ${setting.id}`);
			} else if (!signed && size === 2 && max > 65535) {
				throw new Error(`Max value out of bounds: ${driver.id}, ${setting.id}`);
			}

			else if (signed && size === 4 && max > 2147483647) {
				throw new Error(`Value cannot be signed: ${driver.id}, ${setting.id}`);
			} else if (!signed && size === 4 && max > 4294967295) {
				throw new Error(`Max value out of bounds: ${driver.id}, ${setting.id}`);
			}
        } else return;
    }
