...
class App {
  ...
  async validate({
    ...
    if (appJson.esm === true && semver.lt(semver.coerce(appJson.compatibility), '12.0.0')) {
      throw new Error('ESM apps require a compatibility of at least >=12.0.0');
    }

    const checkMjs = async (path) => {
      await fs.promises.access(path).then(() => {
        if (semver.lt(semver.coerce(appJson.compatibility), '12.0.0')) {
          throw new Error(`ESM apps require a compatibility of at least >=12.0.0. (${path})`);
        }
      }).catch(err => {
        if (err.code !== 'ENOENT') {
          throw err;
