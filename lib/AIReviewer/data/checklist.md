# Athom Internal App Review Checklist

> **DO NOT EDIT THIS FILE DIRECTLY.**
>
> The canonical source is the internal Google Doc:
> https://docs.google.com/document/d/19p9oVSzUuCYWu_WngZieebJB0-HxSSnPrYcKelq6RhM/edit
>
> To change a rule: update the Google Doc first, then re-derive this file. Edits made directly here will drift away from what the human review team actually enforces, which is the most common source of false-positive AI rejections.
>
> **Severity wording is load-bearing.** The AI parses this file literally:
...
>
> When the Google Doc says "should", do not write "must" here. When it says "not allowed", do not soften to "should avoid". Wording mismatches drive incorrect severity and thus incorrect verdicts.

Last synced: 2026-06-29 (canonical Google Doc).
...
### Duplicate
- Check whether a similar app already exists in the App Store.
- If one exists: the developer should reach out to the existing community developer to cooperate or submit a PR. Submissions that resemble an existing app without explanation are rejected.
- Duplicates of Athom apps are not allowed. If they add functionality Athom does not offer, this is discussable — possibly Athom adds it, or accepts a community PR if the code quality is good enough.
...
- "Should" / "avoid" wording produces **warning** severity. Do not upgrade these to blocker, even if the issue feels significant — the canonical Google Doc uses "should" deliberately, signalling that reviewers approve-with-feedback rather than reject.
