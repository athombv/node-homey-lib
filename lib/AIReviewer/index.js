...
module.exports = class AIReviewer {
   ...
   * @param {object} args.manifest - the parsed app.json
   * @param {string} [args.appId] - app id (display only, appears in prompt header)
  ...
  async review({
    ...
    debug(`review completed in ${((Date.now() - t0) / 1000).toFixed(1)}s model=${result.model} tokens in=${result.tokensUsed.input} out=${result.tokensUsed.output} cacheRead=${result.tokensUsed.cacheRead} cacheCreate=${result.tokensUsed.cacheCreate}`);
  ...
  _buildUserContent({
    ...
    const header = [
      ...
      'Store screenshots are attached as image blocks below.',
      '',
      '---',
      '',
    ].join('\n');
