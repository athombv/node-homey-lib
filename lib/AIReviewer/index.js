...
module.exports = class AIReviewer {
  ...
  async review({
    ...
    if (process.env.HOMEY_AI_REVIEW_DRY_RUN) {
      debug('HOMEY_AI_REVIEW_DRY_RUN set — bypassing model call');
      throw new Error('HOMEY_AI_REVIEW_DRY_RUN is set; the request was built but not sent to the model. Run with DEBUG=homey-lib:AIReviewer to inspect the payload.');
    }
  ...
  _buildUserContent({
    ...
    if (normalizedImages.length > 0) {
      ...
      for (const img of normalizedImages) {
        ...
        const truncated = img.url.length > 200 ? `${img.url.slice(0, 200)}… (${img.url.length} chars total)` : img.url;
        debug(`  image_url [${img.label}]: ${truncated}`);
      }
    }
