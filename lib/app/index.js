...
class App {
	...
	async validate({
		...
		if( Array.isArray(appJson.drivers) ) {
			...
			for( let i = 0; i < appJson.drivers.length; i++ ) {
				let driver = appJson.drivers[i];
				
				// validate if `/drivers/:id` exists
				await this._fileExistsCaseSensitive(`/drivers/${driver.id}`);
		...
		if( appSdk === 1 ) {
			await this._fileExistsCaseSensitive('/app.js');
		}
	...
	async _fileExistsCaseSensitive(filepath) {		
		...
		let exists = await check( filepath );
		if( exists !== true )
