...
App.prototype.validate = function( lvl_publish ){
	...
	if( typeof json.author == 'object' ) {
		...
		if( typeof json.author.email !== 'undefined' && typeof json.author.email !== 'string' )
			error("invalid `author.email` in app.json, must be of type string");
	}

	if( lvl_publish && typeof json.bugs == 'object' ) {
		if( typeof json.bugs.url !== 'undefined' && typeof json.bugs.url !== 'string' )
			error('invalid bugs.url, expected string');
	}
	...
	if( typeof json.flow == 'object' ) {
		for( var column in json.flow ) {
			...
			json.flow[ column ].forEach(function(card, i){
				...
				if( lvl_publish && Array.isArray(card.tokens) ) {
					card.tokens.forEach(function(token, j){
						
						var type = token.type || 'string';
...
function isI18nObject( input ) {
	if( typeof input === 'string' ) return true;
	
	try {
		return ( typeof input['en'] === 'string' );
	} catch( err ) {
		return false;
	}
}
