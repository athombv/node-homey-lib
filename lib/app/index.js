...
App.prototype.validate = function( lvl_publish ){
	...
	if( Array.isArray(json.speech) ) {
		...
		if( !json.speech.en ) return error("English speech property is required");
		if( !json.speech.en.group ) return error("English speech `group` property is required");
		...
		for( var language in json.speech ) {
			...
			//validate elements
			if( !json.speech[ language ].element ) 	return error("speech elements missing for language: " + language);

			let elements = json.speech[ language ].element;
			for (var name in elements) {
				if (name.match(/\b("TIME|REGEX|LOCATION|ZONE|GROUPDEVICE|VP|NP)\b/)) return error("System elements cannot be modified: " + name);
				...
				if (componentNames.indexOf(name) !== -1) return error("speech name used more than once: " + name);
			...
			if( !json.speech[ language ].group ) {
				...
				for (var name in groups) {
					if (typeof groups[name].set != 'string') return error("Group requires set: "+name);
					if (groups[name].set.match(/\|\|/) && (groups[name].set.match(/\&\&/))) return error("Invalid set: " + name+". Sets cannot combine || and &&.");
					
					if (componentNames.indexOf(name) !== -1) return error("speech name used more than once: " + name);
					componentNames.push(name);

					const andGroup = groups[name].set.match(/\&\&/);
					if (andGroup && (typeof groups[name].ordered === 'undefined' || typeof groups[name].allowDisconnect === 'undefined')) return error("Required property missing in &&-group: "+ name);
					if (andGroup &&  typeof groups[name].ordered != 'boolean') 			return error("TypeError: \'ordered\' of "+name+" is not boolean");
					if (andGroup &&  typeof groups[name].allowDisconnect != 'boolean') 	return error("TypeError: \'allowDisconnect\' of "+name+" is not boolean");
					if (andGroup &&  typeof groups[name].capturingGroup != 'boolean') 	return error("TypeError: \'capturingGroup\' of "+name+" is not boolean");
					
					//check if not only optional tokens, or optional tokens in ordered group
					let set = groups[name].set.split(/\s?(\&\&|\|\|)\s?/);
					let optional = 0;
					
					set.forEach( componentName => {
						const match = componentName.match(/\(((?:[\w])+)\)/i);
						if (match) optional++;
					})

					if (optional === set.length) return error("Groups can\'t only have optional components: " + name);
					if (optional > 0 && groups[name].ordered) return error("Cannot use optional tokens in an ordered group");

					namesInSets = namesInSets.concat(set);

					if (groups[name].capturingGroup) capturingGroups.push(name);
				}
				if (capturingGroups.length == 0) return error("No capturing groups: " + language);
				
				namesInSets.forEach( name => {
					if (componentNames.indexOf(name) == -1) error("speech component does not exist: " + name);
				})

				componentNames.forEach( name => {
					if (namesInSets.indexOf(name) == -1) error("speech component created but not used: " + name, true);
				})
			}
		}
	}
