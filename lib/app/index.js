...
App.prototype.validate = function( lvl_publish ){
				...
				driver.settings.forEach(function(setting, j) {
					...
					if( setting.type !== 'group' ) {
						...
						if( typeof setting.value !== allowedDriverSettingsTypes[ setting.type ] )
							return error("invalid type for `driver[" + i + "].settings[" + j + "].value` in app.json, expected type " + allowedDriverSettingsTypes[ setting.type ] );
	...
	if( typeof json.capabilities !== 'undefined' ) {
		...
		for( let capability in json.capabilities ) {
