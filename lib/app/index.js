...
// allowed categories
var categories = [ 'lights', 'video', 'music', 'appliances', 'security', 'climate', 'tools', 'internet' ];

// allowed types for driver settings
var driverSettingsTypes = {
	'text': 'string',
	'password': 'string',
	'textarea': 'string',
	'number': 'number',
	...
	if( Array.isArray(json.drivers) ) {
		json.drivers.forEach(function(driver, i){
			...
			if( Array.isArray(driver.settings) ) {
				driver.settings.forEach(function(setting, j) {
					if( typeof setting.id == 'undefined' )
						return error("missing `driver[" + i + "].settings[" + j + "].id` in app.json");

					if( typeof setting.type == 'undefined' )
						return error("missing `driver[" + i + "].settings[" + j + "].type` in app.json");

					if( typeof setting.value == 'undefined' )
						return error("missing `driver[" + i + "].settings[" + j + "].value` in app.json");

					if( typeof setting.value !== driverSettingsTypes[ setting.type ] )
						return error("invalid type for `driver[" + i + "].settings[" + j + "].value` in app.json, expected type " + driverSettingsTypes[ setting.type ] );
